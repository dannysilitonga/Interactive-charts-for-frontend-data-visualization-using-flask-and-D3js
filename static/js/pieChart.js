function d3PieChart(dataset, datasetBarChart){
    // Set up SVG dimensions and properies
    ...
    // Selecting the div and id pieChart on the index.html template
    const visualization = d3.select('#pieChart')
        ...
        .attr("transform", "translate(" + outerRadius + "," + outerRadius + ")"); //Piechart transformation and transition upon page loading )

    const data = d3.pie() //Creating the data object that will develop the various segment of the pie chart.    
        .sort(null)
        .value(function(d){return d.value;})(dataset);   //Retrieve the pie chart data values from our Fask app, the pie chart where tied to a 'value' key of a JSON object. 
    ...
    d3.selectAll("g.slice") //select slices in the group SVG element (piechart)
        .selectAll("path")
        .transition()
        .duration(200)
        .delay(5)
    ...
    arcs.filter(function(d) { return d.endAngle - d.startAngle > .1; }) //Define slice labels at certain angles.
        ...
        .attr("transform", function(d) { return "translate(" + innerArc.centroid(d) + ")"; }) //Positioning upon and transform
        .text(function(d) { return d.data.category; }); //Append category name on slices

    visualization.append("svg:text") //Append the tile of chart in the middle of the pie chart
        .attr("dy", ".20em")
